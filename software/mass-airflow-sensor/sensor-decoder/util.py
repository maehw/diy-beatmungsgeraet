...
class FlowMeterHla():
    ...
    def decode(self, data):
        if data["type"] == "start":
            ...
            # set start time
            self.temp_frame["start_time"] = data["start_time"]
            
            # mark frame as started
            self.frame_started = True
            # reset frame data
            self.temp_frame["data"]["address"] = None
            self.frame_data = bytearray()

        if data["type"] == "address":
            # TODO: sanity-check if frame has started
        ...
        if data["type"] == "stop":
            ...
            if "I2C address matches": # TODO should depend on I2C_BUS_ADDRESS_SETTING
                if "isacommand": # FIXME: I2C master writes command to sensor: identified by write direction (address)
                    ...
                    elif self.sensor_mode = getMeasurement:
